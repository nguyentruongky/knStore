...
class PINController: knController {
    ...
    let pinTextField = UIMaker.makeTextField(text: "Create PIN",
                                               font: UIFont.systemFont(ofSize: 21),
                                               color: .white)
    
    func makeButton(title: String) -> UIButton {
        let button = UIMaker.makeButton(title: title,
                                       titleColor: .white,
    ...
    var usage = PINUsage.authenticate {
        didSet {
            ...
            if usage == .setup {
                ...
                pinTextField.text = "create_pin".i18n
                return
            }
            ...
            if usage == .confirm {
                dotsView.isHidden = true
                pinTextField.isHidden = false
                pinTextField.text = "confirm_pin".i18n
                return
            }
    ...
    let cancelButton: UIButton = UIMaker.makeButton(title: "Cancel",
                                                      titleColor: .white,
                                                      font: .systemFont(ofSize: 15))
    let deleteButton: UIButton = UIMaker.makeButton(title: "Delete",
                                                      titleColor: .white,
                                                      font: .systemFont(ofSize: 15))
    ...
    let dotsView = UIView()
    let backButton: UIButton = UIMaker.makeButton(image: UIImage(named: "back_arrow"))
    let titleLabel = UIMaker.makeLabel(text: "Authentication",
                                         color: .white)
    override func setupView() {
        ...
        deleteButton.verticalSpacing(toView: numberPad,
                                     space: DeviceType.IS_IPHONE_5_OR_LESS ?
                                        numberPadding / 2 :  numberPadding )
