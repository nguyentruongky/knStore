...
class AnalogClockView: UIView {
    ...
    var running: Bool {
        ...
        set(isRunning) {
            ...
            if running {
                ...
                RunLoop.main.add(clockTimer!, forMode: RunLoop.Mode(rawValue: "NSRunLoopCommonModes"))
            }
        }
    }
    ...
    func setup() {
        enterForegroundNotification = NotificationCenter.default.addObserver(forName: UIApplication.willEnterForegroundNotification, object: nil, queue: nil, using: { (note) in
            self.setTimeAnimated(true)
        })
        
        enterBackgroundNotification = NotificationCenter.default.addObserver(forName: UIApplication.didEnterBackgroundNotification, object: nil, queue: nil, using: { (note) in
...
//        let labelString = ["12", "3", "6", "9"]
    ...
    func animateHandView(_ handView: UIView, toAngle: CGFloat, duration: Double) {
        ...
        if duration > 0.4 {
            damping = 0.6
            UIView.animate(withDuration: duration, delay: 0, usingSpringWithDamping: damping, initialSpringVelocity: 0.8, options: UIView.AnimationOptions.curveEaseIn, animations: { 
