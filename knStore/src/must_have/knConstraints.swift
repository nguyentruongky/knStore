...
extension UIView {
    ...
    @discardableResult
    public func size(width w: CGFloat,
                     height h: CGFloat,
                     multiplier: CGFloat = 1,
                     priority: UILayoutPriority = .required,
                     isActive: Bool = true)
        -> (widthConstraint: NSLayoutConstraint, heightConstraint: NSLayoutConstraint) {
            let widthConstraint = width(w, priority: priority, isActive: isActive)
            let heightConstraint = height(h, priority: priority, isActive: isActive)
            return (widthConstraint, heightConstraint)
    }
...
extension UIView {
    func animate(duration: Double = 1, dampingRation: CGFloat = 0.4) {
        UIViewPropertyAnimator(duration: duration, dampingRatio: dampingRation) { [weak self] in
            self?.layoutIfNeeded()
        }.startAnimation()
    }
}
